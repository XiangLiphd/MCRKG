...
class ContrastiveHead(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim=256, pool_type='mean'):
        super().__init__()
    ...
    def forward(self, node_vecs):
        if self.pool_type == 'mean':
            graph_vec = node_vecs.mean(dim=1)
        elif self.pool_type == 'max':
            graph_vec = node_vecs.max(dim=1)[0]
        else:
            graph_vec = node_vecs[:, 0]  # CLS token
        return self.mlp(graph_vec)


class CaseRetriever(nn.Module):
    def __init__(self, case_db_path, model_name='/home/inspur/lx/models/chinese-reberta-wwm'):
        super().__init__()
    ...
    def _load_cases(self, path):
        ...
        case_vecs = outputs[0].mean(dim=1)

        return [{
            'vec': vec,
            'label': c['fault_reason'],
            'graph_data': c['graph']
        } for c, vec in zip(cases, case_vecs)]

    def retrieve(self, query_text, query_label, topk=10):
        device = next(self.bert.parameters()).device
        inputs = self.tokenizer(query_text, return_tensors='pt')
        inputs = {k: v.to(device) for k, v in inputs.items()}

        with torch.no_grad():
            query_vec = self.bert(**inputs)[0].mean(dim=1)

        sims = F.cosine_similarity(query_vec, [c['vec'] for c in self.case_db][0].to(device))
        indices = np.argsort(-sims.cpu().numpy())
        ...
        for idx in indices:
            ...
            if self.case_db[idx]['label'] == query_label:
                ...
                if len(neg) < topk: neg.append(self.case_db[idx])
        return pos, neg
...
class GNN_Message_Passing(nn.Module):
    def __init__(self, args, k, n_ntype, n_etype, input_size, hidden_size, output_size,
        ...
        self.gnn_layers = nn.ModuleList([ANAGATLayer(hidden_size, hidden_size) for _ in range(k)])
...
class GNN(nn.Module):
    def __init__(self, args, k, n_ntype, n_etype, sent_dim,
                 ...
                 init_range=0.02, contrastive=True):
        super().__init__()
        ...
        if contrastive:
            self.contrast_head = ContrastiveHead(concept_dim, pool_type=args.pool_type)
            self.case_retriever = CaseRetriever(
                args.case_db_path
            )
            self.temperature1 = 0.1
            self.temperature2 = 0.1
    ...
    def forward(self, sent_vecs, concept_ids, node_type_ids, node_scores, adj_lengths, adj, emb_data=None, cache_output=False, return_contrastive=False):
        ...
        if self.contrastive and return_contrastive:
            node_vecs = gnn_output  # [batch_size, num_nodes, dim]
            graph_vecs = self.contrast_head(node_vecs)

            return {
                'logits': logits,
                'node_vecs': node_vecs,
                'graph_vecs': graph_vecs
            }
        else:
...
class LM_GNN(nn.Module):
    def __init__(self, args, model_name, model_path, k, n_ntype, n_etype,
                 n_concept, concept_dim, concept_in_dim, n_attention_head,
                 fc_dim, n_fc_layer, p_emb, p_gnn, p_fc,
                 pretrained_concept_emb=None, freeze_ent_emb=True,
                 init_range=0.0, encoder_config={}):
        super().__init__()
        self.train_qid2text = self._load_qid_mapping(args.train_statements)
        self.encoder = TextEncoder(model_name, model_path,**encoder_config)
        self.decoder = GNN(args, k, n_ntype, n_etype, self.encoder.sent_dim,
                                        n_concept, concept_dim, concept_in_dim, n_attention_head,
                                        fc_dim, n_fc_layer, p_emb, p_gnn, p_fc,
                                        pretrained_concept_emb=pretrained_concept_emb, freeze_ent_emb=freeze_ent_emb,
                                        init_range=init_range)

    def _load_qid_mapping(self, statement_path):
        qid2text = {}
        with open(statement_path, 'r') as f:
            for line in f:
                data = json.loads(line)
                qid2text[data['id']] = data['question']['stem'] + ' ' + \
                                           ' '.join([c['text'] for c in data['question']['choices']])
        return qid2text

    def forward(self, *inputs, layer_id=-1, cache_output=False, detail=False, contrastive=False):
        """
        ...
        *lm_inputs, concept_ids, node_type_ids, node_scores, adj_lengths, edge_index, edge_type = _inputs
        ...
        if contrastive:
            model_output = self.decoder(sent_vecs.to(node_type_ids.device),
                                        concept_ids,
...
class LM_GNN_DataLoader(object):

    def __init__(self, args, train_statement_path, train_adj_path,
                 dev_statement_path, dev_adj_path,
                 test_statement_path, test_adj_path,
                 batch_size, eval_batch_size, device, model_name, model_path, max_node_num=200, max_seq_length=128,
        ...
        print ('train_statement_path', train_statement_path)
        self.train_qids, self.train_labels, *self.train_encoder_data = load_input_tensors(train_statement_path, model_type, model_path, max_seq_length,'def')
        self.dev_qids, self.dev_labels, *self.dev_encoder_data = load_input_tensors(dev_statement_path, model_type, model_path, max_seq_length,'def')
        ...
        *self.dev_decoder_data, self.dev_adj_data = load_sparse_adj_data_with_contextnode(dev_adj_path, max_node_num, num_choice, args)
        ...
        if test_statement_path is not None:
            self.test_qids, self.test_labels, *self.test_encoder_data = load_input_tensors(test_statement_path, model_type, model_path, max_seq_length,'def')
...
class ANAGATLayer(nn.Module):
    def __init__(self, in_dim, out_dim, num_hops=2): 
        ...
        self.W_importance = nn.Linear(2 * out_dim, 1)
    ...
    def forward(self, x, edge_index):
        ...
        combined = torch.cat([outputs[0], outputs[1]], dim=1)
        s1 = torch.sigmoid(self.W_importance(combined))
        s2 = 1 - s1
        return s1 * outputs[0] + s2 * outputs[1]
