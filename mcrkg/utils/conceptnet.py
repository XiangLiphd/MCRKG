...
def extract_chinese(conceptnet_path, output_csv_path, output_vocab_path):
    """
...
def construct_chinese_graph(cpnet_csv_path, cpnet_vocab_path, output_path, prune=True):
    print('generating ConceptNet graph file...')

    nlp = spacy.load('zh_core_web_sm')  # en_core_web_sm
    nltk_stopwords = nlp.Defaults.stop_words

    blacklist = set(["uk", "us", "take", "make", "object", "人"])  # issue: mismatch with the blacklist in grouding.py
    ...
    with open(cpnet_csv_path, "r", encoding="utf-8") as fin:
...
if __name__ == "__main__":
    ...
    extract_chinese("./data/cpnet/x/conceptnet-assertions-5.6.0.csv", "./data/cpnet/conceptnet.zh.csv", './data/cpnet/concept.txt')